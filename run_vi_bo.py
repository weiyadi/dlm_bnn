...
import models
import metrics
from helpers import train_epoch_vi_bo, get_kwargs, eval_epoch_vi, eval_epoch_vi_criterion
import yaml
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bm', type=float, default=100., help='Bounds for the 2nd order norm of mean')
    parser.add_argument('--bv', type=float, default=.25, help='Bounds for the infinity order norm of mean')
    ...
    vi_model = VIFFGModel(model_cfg.base, priors, 'naive', cfg, pos_mean=pos_mean, pos_rho=pos_rho, *model_args,
    ...
    for epoch in range(epochs):
        train_obj, _, train_kl = train_epoch_vi_bo(vi_model, optimizer, criterion, train_loader, bm=args.bm, bv=args.bv,
                                                   L=L_train, regression=regression, scheduler=scheduler)
        other_obj, _ = eval_epoch_vi_criterion(vi_model, other_criterion, train_loader, L=L_train, beta=0.,
